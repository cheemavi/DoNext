...
import handleAddNewTask from './TaskInput.jsx';
...
const [isRecording, setisRecording] = useState(false);
  ...
  if (!browserSupportsSpeechRecognition) {
    return   <div>
      <button id='speech-text-btn' className='cursor-not-allowed shadow font-semibold text-sm whitespace-nowrap  py-3 px-4 bg-violet-300 rounded-sm dark:bg-pink-400 dark:text-gray-900 dark:hover:bg-pink-700 dark:hover:text-slate-900' disabled={true}>
...
const handleSpeechText = (t)=>{
    ...
    if (isRecording && toggleButton.hasChildNodes){
      ...
      //append the not recording button and remove the recording button
...
// <button className = "cursor-pointer shadow font-semibold text-sm whitespace-nowrap hover:bg-violet-700  py-3 px-4 bg-violet-300 rounded-sm " onClick = {SpeechRecognition.startListening}>{'Start'}</button>
     // <button className = "cursor-pointer shadow font-semibold text-sm whitespace-nowrap hover:bg-violet-700  py-3 px-4 bg-violet-300 rounded-sm " onClick= {()=>{SpeechRecognition.startListening(); handleNewT(transcript)}}>{'Stop'}</button>
                  




  //the speech to text UI:
